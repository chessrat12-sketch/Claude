...
- ✅ **LLM 실행**: 관찰→프롬프트→모델→행동 배선. 각 에이전트의 `reason`
  (행동 이유)이 💭 이벤트로 화면에 실시간 노출. Groq/OpenRouter/NVIDIA/
  Gemini/xAI/Anthropic/RunPod/로컬 Ollama 등 OpenAI 호환·Anthropic
  백엔드 전부 지원.
- ✅ **10 → 100+ 규모**: 공유 무료 API용 순차 페이싱(`--llm-gap-ms`)과, 자체
  소유 엔드포인트(RunPod vLLM 등)용 **동시 호출**(`--llm-concurrency`)을
  모두 지원. 실측 20배 속도 향상(에이전트 20명 기준), 100명도 확인.
